...
for (const client of ['codex','claude']) test(`offline bundled MCP ${client} initialization, tools, evidence and lifecycle`,async t => {
  ...
  const lodgeTool = tools.find(tool => tool.name === 'lodge_complaint');
  assert.equal(lodgeTool.inputSchema.properties.dry_run.type,'boolean');
  assert.equal(lodgeTool.annotations.readOnlyHint,false);
  ...
  const input = {text:'Offline friction in /tmp/client',evidence:'Observed during local test',severity:'major',tags:['tools'],command:'bash /tmp/client',stderr:'API_TOKEN=private',exit_status:2};
  const draft = await call('lodge_complaint',{...input,dry_run:true});
  assert.equal(draft.dry_run,true); assert.equal(draft.changed,false);
  assert.equal(draft.preview.text,'Offline friction in /tmp/...');
  assert.deepEqual(draft.preview.context,{note:input.evidence,command:'bash /tmp/...',stderr:'API_TOKEN=[REDACTED]',exit_status:2});
  assert.ok(!fs.existsSync(path.dirname(inspect.storage.path)));
  for (const stderr of ['HOME=a\nPATH=b','x'.repeat(4097)]) assert.equal((await call('lodge_complaint',{...input,stderr,dry_run:true})).error.code,'invalid_input');
  assert.ok(!fs.existsSync(path.dirname(inspect.storage.path)));
  const lodged = await call('lodge_complaint',input);
  ...
  assert.equal(id,draft.preview.id);
  const saved = (await call('get_complaint',{complaint_id:id})).complaint;
  assert.deepEqual(draft.preview,Object.fromEntries(Object.keys(draft.preview).map(key => [key,saved[key]])));

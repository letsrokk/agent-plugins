...
def resolve_storage(
    ...
    *,
    ...
    if explicit_file is not None:
        return StorageContext(project, root, explicit_file, "project", None)
    ...
    if environment_file:
        ...
        return StorageContext(project, root, journal_path, "project", None)

    project_config = _config_path(root)
    project_scope = _read_scope(project_config)
    if project_scope is not None:
        return _storage_context(project, root, resolved_home, project_scope, project_config)

    user_config = _config_path(resolved_home)
    user_scope = _read_scope(user_config)
    if user_scope is not None:
        return _storage_context(project, root, resolved_home, user_scope, user_config)

    return _storage_context(project, root, resolved_home, "project", None)
...
def _config_path(base: Path) -> Path:
    return base / ".codex" / _CONFIG_NAME
...
def _storage_context(
    ...
    return StorageContext(
        project=project,
        project_root=root,
        journal_path=journal_base / ".codex" / _JOURNAL_NAME,
        scope=scope,

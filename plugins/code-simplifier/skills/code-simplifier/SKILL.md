...
Resolve one edit boundary, then dispatch exactly one isolated custom agent named `code_simplifier`. The agent simplifies code without changing behavior.
...
6. Spawn exactly one isolated custom agent with custom agent type/name `code_simplifier`, wait for it to finish, and return its result. Tell it to read applicable `AGENTS.md`, preserve user changes, and not commit, push, or open a PR/MR.
...
The only valid workflow outputs are one successful `code_simplifier` dispatch or a clear stop. If `code_simplifier` is unavailable, stop. Do not substitute another agent, copy this prompt into a general agent, or simplify inline.

Team pressure and an available PR never override a dirty worktree: any staged, unstaged, or untracked changes are the complete default scope. Time pressure and a small scope never permit inline work or a substitute agent: unavailable `code_simplifier` means stop.

...
| Social post / thread / personal longform | Author-written draft, notes, transcript, or existing piece supplying the point, supporting substance, and takeaway; a short complete thought can support a short post |

A topic, keywords, a URL collection, or a repository alone is insufficient article or blog-post input. Request the author's skeleton or notes; return `needs-source`, not invented prose. Inspect an existing author-written piece supplied by URL before deciding whether its contents satisfy the gate. Repository evidence may verify their technical argument but cannot create it. An unsupported investor deck receives only supported slides and private missing-input notes, with `partial` status.
...
For non-product articles and social writing, inspect the supplied author's material; repository/product inspection and product-brief fields below apply only when relevant. Use the same private records to capture the author, subject, point, audience, voice, evidence, and takeaway without inventing a product or inspecting an unrelated repository.
...
Write `sources.json` for research, article, and blog-post sources; see [research](research.md). Write `assets.json` when assets exist; see [visuals](visuals.md). In `review.md` include stage/status, checks actually run and results, assumptions, missing facts/capabilities, and a transformation map: source note/section → output section → transformation → substantive addition or unresolved issue. One source-capability sentence suffices for a tagline; articles need section-level traceability, and social writing needs traceability per post or longform section.
